...
1. **用途卡片首页**：六张卡片分别直达城市尽调、企业库、产业链、全国地图、区域比较和证据口径，并支持用途分类筛选。
2. **本地优先地图**：ECharts、全国省界和 34 份省级地图全部随项目提供；运行时 CDN 仅为降级备份。
3. **全国研究入口**：右侧默认显示操作步骤与重点城市入口，不再用随机省份占据初始详情。
4. **南通一键直达**：首页与全国入口均可直接打开南通量化深度样板。
5. **移动端双视图**：手机端提供“地图与筛选 / 城市详情”切换；选中城市后自动进入详情。
6. **港澳固定入口**：地图上始终显示“香港 / 澳门”快捷入口，避免小面积区域在全国比例尺下难以点击。
7. **台湾县市下钻**：补充 20 个台湾县市的本地边界，县市可见、可点、可与企业画像联动。
8. **城市优先信息层级**：城市定位 → KPI → 产业证据 → 企业 → 城市画像 → 省域背景。
9. **可访问性基础**：增加跳转链接、语义化标签、键盘焦点、ARIA 状态和 reduced-motion 支持。
...
本版本还在真实浏览器中验证了桌面与 390px 手机布局、用途筛选和跨卡片跳转、江苏市界、台湾 20 县市边界实点、港澳入口、城市自动切换、企业库加载、手机返回地图以及浏览器控制台无错误。
