...
## V2.2 决策流程 / Decision workflows

V2.2 保留 V2.1 的 Explore、地图、城市详情、企业表格和全局搜索，只重构 Investor、Policy 和 Learn 三个任务工作台：

- **Investor / 投资论点工作台**：研究设置 → 用户自建可比组 → 证据与反证 → Markdown 研究备忘录。系统不再自动加入固定城市，也不再用抽象总分替代可比性和数据缺口。
- **Policy / 产业链招商行动台**：任务定义 → 强/弱/待补证据诊断 → 带推荐理由与排除项的结构候选 → 用户维护的项目管线与招商简报。候选不表示企业投资、迁移或扩产意向。
- **Learn / 产业研究案例实验室**：Observe → Diagnose → Decide → Defend → Debrief。学习者必须先选择证据、提交判断、反证和推翻条件，之后才能查看过程完成度反馈。
- **共同机制**：步骤导航、单一主要行动、右侧数据就绪度与来源栏、`atlas.v22.*` 浏览器本地保存、Markdown 导出、诚实缺失状态和手机单列布局。

V2.2 是任务工作流层，不新增或伪造项目、估值、企业意向和政策执行事实。招商项目管线和案例练习记录默认只保存在当前浏览器。
...
│   ├── v22-app.js             # Investor / Policy / Learn 决策流程增量层
